...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /users/{userId} {
      ...
      // - El staff puede editar cualquier campo de cualquier usuario
    ...
    match /workouts/{workoutId} {
      // Lectura: el cliente asignado o cualquier staff
      allow read: if isAssignedClient(resource.data.clientId) || isStaff();
      
      // Escritura (crear, actualizar, eliminar): solo staff
...
|-----------|---------|----------|---------------|-------------|
| `users` | Todos auth | Propio usuario | Propio usuario (sin role) / Staff | Solo Admin |
| `workouts` | Cliente asignado / Staff | Solo Staff | Solo Staff | Solo Staff |
| `diets` | Cliente asignado / Staff | Solo Staff | Solo Staff | Solo Staff |
| `progress_logs` | Propio cliente / Staff | Solo cliente | Solo cliente | Solo cliente |
| `messages` | Solo participantes | Solo sender | Solo receiver (isRead) | ❌ No permitido |
