 ...
 * Usa \b (word boundary) para asegurar que sea la función t() y no parte de
 * otra palabra como import(), closest(), createElement(), get(), etc.
 */
function extractUsedKeys(dir: string): string[] {
  ...
  for (const file of files) {
    ...
    // Busca t('key') o t("key") - con \b para evitar falsos positivos
    // como import(), closest(), createElement(), get(), etc.
    const regex = /\bt\(['"]([^'"]+)['"]\)/g;
    let match: RegExpExecArray | null;
    while ((match = regex.exec(content)) !== null) {
      if (match[1]) {
...
function getKeysByContext(dir: string): { ssrKeys: string[]; clientKeys: string[] } {
  ...
  for (const file of files) {
    ...
    if (ext === '.astro') {
      ...
      // Keys en script (cliente) - con \b para evitar falsos positivos
      const clientRegex = /\bt\(['"]([^'"]+)['"]\)/g;
      while ((m = clientRegex.exec(scriptContent)) !== null) {
        if (m[1]) clientKeys.push(m[1]);
      }
    } else {
      // Archivos .ts/.tsx - asumimos cliente, con \b para evitar falsos positivos
      const regex = /\bt\(['"]([^'"]+)['"]\)/g;
      let m: RegExpExecArray | null;
